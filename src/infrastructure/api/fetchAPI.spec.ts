import { fetchAPI } from './fetchAPI';

global.fetch = jest.fn();

const mockFetch = global.fetch as jest.MockedFunction<typeof fetch>;

describe('fetchAPI', () => {
  beforeEach(() => {
    mockFetch.mockClear();
  });

  describe('GET requests', () => {
    it('should successfully fetch transactions from the server', async () => {
      // Given: Mock the response data
      const mockTransactions = {
        transactions: [
          {
            id: 'test-123',
            amount: -50.5,
            payee: 'Test Merchant',
            timestamp: 1729872000000,
            memo: 'Test transaction',
          },
        ],
      };

      // Given: Mock the fetch response
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => mockTransactions,
      } as Response);

      // When: Call the function with GET method (default)
      const result = await fetchAPI();

      // Then: Verify the result
      expect(result).toEqual(mockTransactions);

      // Then: Verify fetch was called with correct parameters
      expect(mockFetch).toHaveBeenCalledTimes(1);
      expect(mockFetch).toHaveBeenCalledWith('/api/transactions', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      });
    });

    it('should fetch with custom URL', async () => {
      // Given: Mock response
      const mockData = { data: 'test' };
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => mockData,
      } as Response);

      // When: Call with custom URL
      const result = await fetchAPI({ url: '/api/custom-endpoint' });

      // Then: Verify correct URL was used
      expect(result).toEqual(mockData);
      expect(mockFetch).toHaveBeenCalledWith('/api/custom-endpoint', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      });
    });
  });

  describe('POST requests', () => {
    it('should successfully post a transaction', async () => {
      // Given: Mock transaction data to send
      const newTransaction = {
        amount: -25.0,
        payee: 'Coffee Shop',
        timestamp: Date.now(),
        memo: 'Morning coffee',
      };

      // Given: Mock the response
      const mockResponse = {
        transaction: { ...newTransaction, id: 'new-123' },
      };

      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => mockResponse,
      } as Response);

      // When: Call the function with POST method
      const result = await fetchAPI({
        url: '/api/transactions',
        method: 'POST',
        body: newTransaction,
      });

      // Then: Verify the result
      expect(result).toEqual(mockResponse);

      // Then: Verify fetch was called with correct parameters
      expect(mockFetch).toHaveBeenCalledTimes(1);
      expect(mockFetch).toHaveBeenCalledWith('/api/transactions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(newTransaction),
      });
    });

    it('should handle POST without body', async () => {
      // Given: Mock response
      const mockResponse = { success: true };
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => mockResponse,
      } as Response);

      // When: Call POST without body
      const result = await fetchAPI({
        url: '/api/transactions',
        method: 'POST',
      });

      // Then: Verify no body was sent
      expect(result).toEqual(mockResponse);
      expect(mockFetch).toHaveBeenCalledWith('/api/transactions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      });
    });
  });

  describe('Error handling', () => {
    it('should throw an error when the API response is not ok', async () => {
      // Given: Mock a failed response
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 404,
        statusText: 'Not Found',
      } as Response);

      // When & Then: Verify a user-friendly error message is thrown
      await expect(fetchAPI()).rejects.toThrow(
        'Unable to fetch data. Please try again later.',
      );

      // Then: Verify fetch was called
      expect(mockFetch).toHaveBeenCalledTimes(1);
    });

    it('should catch any API error and return a user-friendly and controled error message', async () => {
      // Given: Mock an unexpected error from the API
      const unexpectedError = new Error('Unexpected server error');
      mockFetch.mockRejectedValueOnce(unexpectedError);

      // When & Then: Verify a custom error message is thrown
      await expect(fetchAPI()).rejects.toThrow(
        'Unable to fetch data. Please try again later.',
      );

      // Then: Verify fetch was called
      expect(mockFetch).toHaveBeenCalledTimes(1);
    });
  });
});
